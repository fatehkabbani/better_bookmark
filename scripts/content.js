...
function createListItem(title, url, id) {
  ...
  } " alt="image" class="no-image">`;
  li.innerHTML = `<span class="imgandlinkspan">${image}${a}</span><span class="buttons">${buttonEdit}${buttonDelete}</span>`;
...
document
  .getElementById("create_new_bookmark")
  .addEventListener("click", createNewBookMark);
...
document
  .getElementById("create_new_folder")
